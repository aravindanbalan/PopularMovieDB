...
public class ImageLoader {
    ...
    ImageLoader(Context context, OkHttpClient client) {
        mPicasso = initPicasso(context, client);
    }

    //FIXME need big time help in improving the picasso image loading. Facing some OutOfMemoryExceptions then and there and page goes blank. Tried all possible things.
    //FIXME I feel its because of recyclerview poping out elements from the grid view each time and loading new images each time. Need guidance.
    // Also tried setting largeHeap = true in application so that the LRU cache uses a large portion of the heap size. still no improvement
    private Picasso initPicasso(Context context, OkHttpClient okHttpClient) {
        ...
        LruCache mCache = new LruCache(context);
